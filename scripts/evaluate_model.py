...
def parse_args():
    ...
    parser.add_argument('--mode', choices=['NoBRS', 'RGB-BRS', 'DistMap-BRS',
                                         'f-BRS-A', 'f-BRS-B', 'f-BRS-C'], default='NoBRS',
                        help='')
    ...
    group_iou_thresh.add_argument('--target-iou', type=float, default=1.00,
    ...
    parser.add_argument('--model-name', type=str, default='MFP',
                        help='The model name that is used for making plots.')
    parser.add_argument('--config-path', type=str, default='./config.yml',   # Path to Configuration file
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    iou_thrs = np.array([0.85, 0.90, 0.95, 0.99, 1.00]).tolist()
